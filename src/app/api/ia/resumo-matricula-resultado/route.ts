...
function getSupabaseAdmin() {
  ...
  if (!url || !serviceKey) {
    throw new Error(
      "SUPABASE_SERVICE_ROLE_KEY não configurado. Necessário para o callback de resumo_matricula."
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      // fallback por nome de campo convencional
      if (!pdfFile) {
    ...
    if (statusNormalized === "erro") {
      const errMsg = payload.error_message || payload.erro || "Erro no processamento do resumo de matrícula";
      updates.resultado_final = {
        ...(payload.resultado_final || {}),
        erro: errMsg,
    ...
    // Upload de PDF/DOC via multipart (prioriza binário)
    const relatorioId = payload.relatorio_id;
    if (pdfFile) {
      const timestamp = Date.now();
      const ext = pdfFile.name.split(".").pop() || "pdf";
      const fileName = `resumo-matricula-${relatorioId}-${timestamp}.${ext}`;
    ...
    if (docFile) {
      ...
      const fileName = `resumo-matricula-doc-${relatorioId}-${timestamp}.${ext}`;
    ...
    if (updateError) {
      console.error("Erro ao atualizar relatorio resumo_matricula:", updateError, {
    ...
    return NextResponse.json({
      ...
      message:
        statusNormalized === "concluido"
          ? "Resumo de matrícula concluído registrado com sucesso"
          : "Resumo de matrícula com erro registrado com sucesso",
      data: { relatorio_id: payload.relatorio_id, status: statusNormalized },
    });
  } catch (error) {
    console.error("Erro no endpoint resumo-matricula-resultado:", error);
    return NextResponse.json(
...
export async function GET() {
  return NextResponse.json({
    success: true,
    message: "Endpoint de resultado do resumo de matrícula funcionando",
    endpoint: "/api/ia/resumo-matricula-resultado",
  });
}
